...
// Set these via Firebase environment config / secrets — do not hardcode.
const kUserKey = process.env.ONESIGNAL_USER_KEY;
const kAPIKey = process.env.ONESIGNAL_API_KEY;
...
exports.addUser = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const createdUser = await client.createUser(
      "b30d5d9c-cc8e-46fd-ac60-d3cd07b8fd9f",
      user,
    );
...
exports.deleteUser = functions.https.onCall(async (data, context) => {
  ...
  const url = `https://api.onesignal.com/apps/b30d5d9c-cc8e-46fd-ac60-d3cd07b8fd9f/users/by/external_id/${data.user_id}`;
